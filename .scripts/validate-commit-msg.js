...
function validate(msg) {
    ...
    var type = match[1], scope = match[2], subject = match[3];
    if (!TYPES.includes(type)) {
        return {
            valid: false,
            reason: "Type '".concat(type, "' is not allowed. Allowed: ").concat(TYPES.join(', '))
        };
    }
    if (!SCOPES.includes(scope)) {
        return {
            valid: false,
            reason: "Scope '".concat(scope, "' is not allowed. Allowed: ").concat(SCOPES.join(', '))
        };
    }
    if (!subject || subject.length > SUBJECT_MAX_LENGTH) {
        return {
            valid: false,
            reason: "Subject must be non-empty and <= ".concat(SUBJECT_MAX_LENGTH, " characters.")
        };
    }
