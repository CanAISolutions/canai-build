...
Establish a secure, robust, and PRD-aligned authentication layer for both frontend and Make.com automation flows using Memberstack JWTs. This plan is strictly MVP-focused and maps directly to PRD.md requirements for authentication, automation, and user journey support (F1–F9).
...
- **Unified Middleware:** Accepts JWTs from both frontend (`Authorization`) and Make.com (`x-memberstack-token`)
...
> - Confirm you have the latest Memberstack public key and know how to rotate it.
...
#### Success Criteria
- Middleware validates JWTs strictly (signature, issuer, audience, expiration).
...
const memberstackMiddleware = async (req, res, next) => {
  ...
  try {
    const decoded = jwt.verify(token, process.env.MEMBERSTACK_PUBLIC_KEY, {
      algorithms: ['RS256'],
      issuer: 'memberstack.com',
    });
...
const makeAuthMiddleware = (req, res, next) => {
  ...
  if (!isValid) {
    return res.status(403).json({ error: 'Invalid Make.com signature', code: 'MAKE_SIGNATURE_INVALID' });
  }
  next();
};
```
...
> **Phase Gate: Before you start:**
> - Ensure router logic is clear and does not introduce ambiguity between frontend and Make.com flows.
...
- **Unified Middleware:** Accept JWTs from both frontend and Make.com for simplicity and maintainability.
...
- **Scope Creep:** Stick to MVP—avoid adding analytics, advanced refresh, or non-Make.com automations.
...
- Make.com scenario JSONs (add_client, add_project, admin_add_project, SAAP Update Project Blueprint)
- Memberstack JWT documentation

---

## Maintenance Notes
- **Dependency Updates:** Regularly run `npm audit` or use tools like Dependabot to identify and patch vulnerabilities in dependencies. Schedule monthly reviews to ensure the system remains secure.
