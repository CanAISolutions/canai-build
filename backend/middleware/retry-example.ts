...
// Helper function to safely access error status
...
async function basicExample() {
  ...
  try {
    ...
    console.log('Failed after retries:', getErrorMessage(error));
  }
}
...
async function customConfigExample() {
  ...
  const config: Partial<RetryConfig> = {
    ...
    onRetry: (error, attempt, delay) => {
      console.log(
        `Retry attempt ${attempt} after ${delay}ms delay. Error: ${getErrorMessage(error)}`
      );
    },
    ...
    onFailure: (error, attempts) => {
      console.log(
        `Failed after ${attempts} attempts: ${getErrorMessage(error)}`
      );
    },
  };
  ...
  try {
    ...
    console.log('Final failure:', getErrorMessage(error));
  }
}
...
async function circuitBreakerExample() {
  ...
  const retryConfig: Partial<RetryConfig> = {
    ...
    shouldRetry: error => {
      ...
      return message.includes('ECONNRESET') || message.includes('ENOTFOUND');
    },
  };
  ...
  try {
    ...
    console.log('First call failed:', getErrorMessage(error));
  }
  ...
  try {
    ...
    console.log('Second call failed:', getErrorMessage(error));
  }
  ...
  try {
    ...
    console.log('Third call failed:', getErrorMessage(error));
  }
}
...
async function serviceWrapperExample() {
  ...
  try {
    ...
    console.log('GPT-4o failed:', getErrorMessage(error));
  }
}
...
async function errorClassificationExample() {
  ...
  for (const testCase of testErrors) {
    ...
    console.log(
      `${testCase.description}: ${shouldRetry ? 'Will retry' : 'Will not retry'}`
    );
  }
}
